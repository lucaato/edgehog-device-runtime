        ...
        match destination_type {
            STORAGE_TARGET => {
                if destination.is_empty() {
                    // when a file is downloaded this should be the file name
                    info!("destination for storage target is empty");
                }
...
mod tests {
    ...
    pub(crate) fn download_req() -> Download<'static> {
        Download {
            ...
            destination: Destination::Storage {
                name: Cow::Owned(PathBuf::from("testfile.txt")),
            },
        }
    }
    ...
    #[rstest]
    #[case(("storage", "testfile.txt"), Destination::Storage { name: Path::new("testfile.txt").into() })]
