...
impl<F> FileStorage<F> {
    ...
    #[instrument(skip_all)]
    pub(crate) async fn init<C>(&self, queue: &Queue, device: C) -> eyre::Result
    where
        C: Client + Send + Sync + 'static,
    {
        let migration_dir = self.dir.join(Self::MIGRATION_DIR);

        trace!(dir = %self.dir.display(), migration = %migration_dir.display(), "initialazing store directories");

        tokio::fs::create_dir_all(&migration_dir)
            .await
            .wrap_err("couldn't create migration directory")?;

        self.finalize_migrations(&migration_dir).await?;

        self.cleanup(queue, device).await?;

        Ok(())
    }

    #[instrument(skip_all)]
    async fn finalize_migrations(&self, migration_dir: &Path) -> io::Result<()> {
        debug!(dir = %migration_dir.display(), "finalizing migrations");

        let mut dir = tokio::fs::read_dir(migration_dir).await?;

        while let Ok(Some(entry)) = dir
            .next_entry()
            .await
            .inspect_err(|error| error!(%error, "couldn't get next migration entry"))
        {
            let path = entry.path();
            let file_type = entry.file_type().await?;

            if !file_type.is_dir() {
                debug!(path = %path.display(), "ignoring migration entry: not a directory");
                continue;
            }

            let Some(uuid) = Self::get_uuid_name(&path) else {
                debug!(path = %path.display(), "ignoring migration entry: not a valid uuid");
                continue;
            };

            let dest = self.dir.join(uuid.to_string());

            if tokio::fs::try_exists(&dest).await.is_ok_and(|b| b) {
                // NOTE if the destination entry already exists it's possible the migration was
                // interrupted before moving the file inside the temporary directory.
                // We skip the move since we could overwrite the file with an empty directory.
                debug!(dest = %dest.display(), "destination entry already exists skipping move");
                continue;
            }

            trace!(path = %path.display(), dest = %dest.display(), "moving migrated directory back to destination");
    ...
    #[instrument(skip_all)]
    async fn cleanup<C>(&self, queue: &Queue, mut device: C) -> io::Result<()>
    where
        C: Client + Send + Sync + 'static,
    {
            ...
            .inspect_err(|error| error!(%error,"couldn't get next dir entry"))
        {
            ...
            if Self::is_migration_entry(&path) {
                continue;
            }
            ...
            self.adapt_entry(&entry, &mut device).await?;
        }

        Ok(())
    }

    fn is_migration_entry(path: &Path) -> bool {
        path.file_name().is_some_and(|f| f == Self::MIGRATION_DIR)
    }

    fn get_uuid_name(path: &Path) -> Option<Uuid> {
        let name = path.file_name()?.to_string_lossy();

        Uuid::try_parse(&name).ok()
    }

    // moves uuid files to a uuid directory
    // ignores all entries that are not files or don't have valid uuid names
    #[instrument(skip_all)]
    async fn adapt_entry<C>(&self, entry: &DirEntry, device: &mut C) -> io::Result<()>
    where
        C: Client + Send + Sync + 'static,
    {
        ...
        if !entry.file_type().await?.is_file() {
            ...
            trace!(path = %file_path.display(), "ignoring entry: not a file");
            return Ok(());
        }

        let Some(uuid) = Self::get_uuid_name(&file_path) else {
            // ignore all entries that are not uuid
            trace!(path = %file_path.display(), "ignoring entry: not a valid uuid");
            return Ok(());
        };

        let temp_dir = self.dir.join(Self::MIGRATION_DIR).join(uuid.to_string());
        let dest_file = temp_dir.join(WriteHandle::MIGRATION_FILE_NAME);
        let final_path = file_path.join(WriteHandle::MIGRATION_FILE_NAME);
        ...
        // update the property in astarte to point to the file path
        StoredFile::update_path(uuid, &final_path, device).await;
        // create temp directory
        tokio::fs::create_dir_all(&temp_dir).await?;
        // move file inside temp directory
        tokio::fs::rename(&file_path, dest_file).await?;
        // move temp directory to previous file location
        tokio::fs::rename(&temp_dir, &file_path).await?;
    ...
    pub(crate) fn file_path(&self, id: &Uuid, name: &Path) -> PathBuf {
        ...
        if name_str.is_none_or(|s| s.is_empty()) {
            // when the name is empty use the default migration file name
            self.dir_path(id).join(WriteHandle::MIGRATION_FILE_NAME)
        } else {
    ...
    #[instrument(skip(self))]
    pub(crate) async fn open_read(&self, id: &Uuid) -> io::Result<tokio::fs::File> {
    ...
    #[instrument(skip_all, fields(id = %opt.id, file_name = %name.display()))]
    ...
    pub(crate) async fn files(&self) -> io::Result<impl Stream<Item = io::Result<StoredFile>>> {
        ...
        let stream = ReadDirStream::new(read_dir).try_filter_map(async |e| {
            ...
            // ignore entries that are not dir or files
            let file_type = e.file_type().await?;
            if !(file_type.is_dir() || file_type.is_file()) {
                warn!(path = %path.display(), "unexpected file type in store directory");
                return Ok(None);
            }

            // TODO compute a correct file size for the entry in the store
            // this requires walking the directory structure
            // NOTE that all the files received by edgehog will already
            // have set the size to the received size from the ServerToDevice request
...
pub(crate) mod tests {
    ...
    use astarte_device_sdk::{
        AstarteData, pairing::api::PairingApi, store::SqliteStore, transport::mqtt::Mqtt,
    };
    ...
    #[tokio::test]
    async fn test_fs_finalize_migrations_no_overwrite() {
        let (store, store_dir) = fs_storage();

        let migration_dir = store_dir.path().join(FileStorage::<()>::MIGRATION_DIR);
        let test_name = Uuid::new_v4();

        tokio::fs::create_dir_all(migration_dir.join(test_name.to_string()))
            .await
            .unwrap();

        let existing_path = store_dir.path().join(test_name.to_string());

        tokio::fs::write(&existing_path, "original content")
            .await
            .unwrap();

        // this must NOT move the migration directory to the store_dir since it would overwrite a file
        store.finalize_migrations(&migration_dir).await.unwrap();

        let content = tokio::fs::read_to_string(&existing_path).await.unwrap();
        assert_eq!(content, "original content");
    }

    #[tokio::test]
    async fn test_fs_finalize_migrations_move_directory() {
        let (store, store_dir) = fs_storage();
        ...
        // this should move the migration directory to the store_dir
        store.finalize_migrations(&migration_dir).await.unwrap();

        let metadata = tokio::fs::metadata(store_dir.path().join(test_name.to_string()))
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_fs_finalize_migrations_and_init() {
        let (store, store_dir) = fs_storage();
        let (queue, _dir) = queue("queue").await;
        ...
        let expected_migrated_file = existing_path.join(WriteHandle::MIGRATION_FILE_NAME);

        device
            .expect_set_property()
            .with(
                predicate::eq("io.edgehog.devicemanager.storage.File"),
                predicate::eq(format!("/{}/pathOnDevice", test_name)),
                predicate::eq(AstarteData::String(
                    expected_migrated_file.to_str().unwrap().to_owned(),
                )),
            )
            .returning(|_, _, _| Ok(()));

        store.init(&queue, device).await.unwrap();

        let metadata = tokio::fs::metadata(&existing_path).await.unwrap();

        assert!(metadata.is_dir());

        let content = tokio::fs::read_to_string(&expected_migrated_file)
            .await
            .unwrap();
